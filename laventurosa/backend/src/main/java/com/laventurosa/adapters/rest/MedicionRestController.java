...
/**
    ...
    public ResponseEntity<List<MedicionDTO>> ultimas() {
        ...
        for (String punto : puntos) {
            medicionRepo.obtenerUltimaPorPunto(punto).map(this::toDTO).ifPresent(resultado::add); // Convierte entidad a DTO para no exponer la base de datos directamente
        }
...
/**
    ...
    public ResponseEntity<List<MedicionDTO>> historial(
            ...
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME)
            LocalDateTime hasta) {
        List<Medicion> mediciones = medicionRepo.obtenerPorRango(desde, hasta);
        // Uso de Streams (Programación Funcional) para transformar los datos rápidamente
